...
public class maincommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings)
    {
        ...
        if(strings.length == 0)
        {
            ...
            commandSender.sendMessage(ChatColor.YELLOW+"[NewyearBlessing]"+ChatColor.RED+messageerror);
            return false;
        }
        if(strings.length == 2)
        {
            ...
            if (Message0.equals("add"))
            {
                ...
                for (String command_ : blessing) {
                    if(command_.equals(player)){
                        ...
                        commandSender.sendMessage(ChatColor.YELLOW+"[NewyearBlessing]"+ChatColor.RED+messagealready);
                        return false;
                    }
                }
                ...
                commandSender.sendMessage(ChatColor.YELLOW+"[NewyearBlessing]"+ChatColor.GREEN+messageup);
            ...
            else if(Message0.equals("looknumber"))
            {
                ...
                commandSender.sendMessage(ChatColor.YELLOW + "[NewyearBlessing]" + ChatColor.GREEN + messagedi+Message1+messageis);
                List<String> blessing = plugin.getConfig().getStringList("Blessing");
        ...
        else if(strings.length == 1)
        {
            ...
            if (Message0.equals("list"))
            {
                ...
                commandSender.sendMessage(ChatColor.YELLOW+"[NewyearBlessing]"+ChatColor.GREEN+messagelist);
                List<String> blessing = plugin.getConfig().getStringList("Blessing");
                int i=0;
                for (String command_ : blessing)
                {
            ...
            else if(Message0.equals("reload"))
            {
                if (commandSender.hasPermission("newyearblessing.commands.reload"))
                {
                    ...
                    commandSender.sendMessage(ChatColor.YELLOW + "[NewyearBlessing]" + ChatColor.GREEN + messagereload);
                }
                else {
                    String messageno = plugin.getConfig().getString("message-no");
                    commandSender.sendMessage(ChatColor.YELLOW + "[NewyearBlessing]" + ChatColor.RED + messageno);
                }
            }
            else
            {
